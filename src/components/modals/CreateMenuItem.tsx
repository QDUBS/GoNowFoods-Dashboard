...
const CreateMenuItem = ({ onClose }: Props) => {
  ...
  const { isLoading, mutate } = useMutation({
    mutationFn: (dish: IDishFormInputs | any) =>
              ...
              <div className="w-8/12">
                ...
                <div className="flex flex-col border-gray rounded-md px-2 py-2 mb-6">
                  ...
                  <select
                    ...
                    {...register("category")}
                  >
                    <option value={AllCategories.FAST_FOOD}>
                      Fast Food
                    </option>
                    <option value={AllCategories.PIZZA}>
                      Pizza
                    </option>
                    <option value={AllCategories.BAKERY_DESSERT}>
                      Bakery & Dessert
                    </option>
                    <option value={AllCategories.HEALTHY_SALAD}>
                      Healthy & Salad
                    </option>
                    <option value={AllCategories.BARBECUE}>
                      Barbecue
                    </option>
                    <option value={AllCategories.BREAKFAST_BRUNCH}>
                      Breakfast & Brunch
                    </option>
                    <option value={AllCategories.SEAFOOD}>
                      Seafood
                    </option>
                    <option value={AllCategories.FAMILY_MEALS}>
                      Family Meals
                    </option>
                    <option value={AllCategories.SOUPS}>
                      Soups
                    </option>
                    <option value={AllCategories.MEAT_PROTEIN}>
                      Meat & Protein
                    </option>
                  </select>
                </div>
